...
我最近在研究 [Apache Superset](https://superset.apache.org)，今天就来说说如何在 [Debian](https://www.debian.org) 上部署 Apache Superset。在开始前先交代下我的 Apache Superset 运行环境，我现在用的电脑是[苹果](https://www.apple.com.cn)的 [MacBook Air](https://www.apple.com.cn/macbook-air/) \(13-inch, 2017\)，操作系统为 [macOS Big Sur](https://www.apple.com.cn/macos/big-sur/) 11.5.2，在 [VirtualBox](https://www.virtualbox.org) 6.1.26 创建的虚拟机上安装的 [Debian Buster](https://www.debian.org/releases/buster/) 10.10.0，然后在 Debian Buster 10.10.0 上部署的 Apache Superset 1.3.1。具体的 Apache Superset 运行环境如下：

- [Debian Buster](https://www.debian.org/releases/buster/) 10.10.0
- [Git](https://git-scm.com) 2.20.1
- [pyenv](https://github.com/pyenv/pyenv) 1.2.24
- [Python](https://www.python.org) 3.7.10
- [Virtualenv](https://virtualenv.pypa.io/en/latest/) 20.7.2
- [MariaDB](https://mariadb.org) 10.3.29
- [Apache HTTP Server](http://httpd.apache.org) 2.4.38
- [PHP](https://www.php.net) 7.3.29-1~deb10u1
- [phpMyAdmin](https://www.phpmyadmin.net) 5.1.1
...
我是用 [netinst](https://www.debian.org/distrib/netinst) 镜像文件安装的 Debian，在安装时我选了 `Debian 桌面环境`、`GNOME` 和 `标准系统工具` 这三个组件。其中安装 `Debian 桌面环境` 和 `GNOME` 是为了我在虚拟机中方便调试。如果是在生产环境中部署 Apache Superset 的话，最小化安装 Debian，只选中 `标准系统工具` 就可以了。

在安装 Debian 时创建的用户为：superset，没有 `sudo` 权限，需要手动授权：

1. 打开终端，登录 [bash](https://www.gnu.org/software/bash/)。
...
Apache Superset 默认使用 [SQLite](https://www.sqlite.org) 部署，但支持多种数据库管理系统部署。我们使用 MariaDB 部署。
...
phpMyAdmin 是一个以 PHP 为基础，以 Web-Base 方式架构在网站主机上的 MySQL 的数据库管理工具，让管理者可用 Web 接口管理 MySQL 数据库。因为 MariaDB 和 MySQL 同源，所以 phpMyAdmin 也可以管理 MariaDB。
...
4. 安装 phpMyAdmin 相关依赖。phpMyAdmin 需要如下 PHP 扩展模块支持：
   ...
   - `php-gettext`：实现了 NLS(Native Language Support) API 国际化支持的 PHP 扩展模块。
